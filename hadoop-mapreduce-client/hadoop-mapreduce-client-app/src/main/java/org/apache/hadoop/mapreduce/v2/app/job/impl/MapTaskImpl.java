...
public class MapTaskImpl extends TaskImpl {
  ...
  private final TaskSplitMetaInfo taskSplitMetaInfo;//split文件,即该map任务需要读取的数据源

  /**
   * 
   * @param jobId 该任务所属job
   * @param partition 该map属于第几个任务
   * @param eventHandler 事件处理器
   * @param remoteJobConfFile job的配置文件/tmp/hadoop-yarn/staging/user/.staging/jobIdString/job.xml
   * @param conf 配置文件对象
   * @param taskSplitMetaInfo split文件,即该map任务需要读取的数据源
   * @param taskAttemptListener
   * @param jobToken
   * @param credentials
   * @param clock
   * @param appAttemptId applicationAttemptId,即MR的中央处理器
   * @param metrics
   * @param appContext
   */
   ...
   * 打印关于split,map数据源文件的所在hdfs路径集合信息
   */
  @Override
