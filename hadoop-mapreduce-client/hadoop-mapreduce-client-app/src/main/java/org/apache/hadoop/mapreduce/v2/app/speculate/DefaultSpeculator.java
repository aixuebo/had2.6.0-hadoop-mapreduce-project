...
public class DefaultSpeculator extends AbstractService implements
  ...
  private Thread speculationBackgroundThread = null;//核心后台推测线程
...
/*   *************************************************************    */
  ...
  public void handleAttempt(TaskAttemptStatus status) {
    long timestamp = clock.getTime();//获取当前时间戳
...
/*   *************************************************************    */
   ...
   *        that we want to fold into the speculation data for this job 尝试任务的状态
   * @param timestamp the time this status corresponds to.  This matters
   *        because statuses contain progress. 当前发生的时间戳
   */
...
//  those jobs that need them.
  ...
  //Add attempt to a given Task. 为task添加一个推测任务
