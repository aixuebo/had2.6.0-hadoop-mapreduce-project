...
public class CLI extends Configured implements Tool {
   ...
   * 2.-status <job-id>
   * 3.-kill <job-id>
   * 4.-counter <job-id> <group-name> <counter-name>
   * 5.-events <job-id> <from-event-#> <#-of-events>]. Event #s start from 1.
   * 6.-history <jobHistoryFile>]
   * 7.-list [all]
   * 8.-kill-task <task-attempt-id>
   * 9.-fail-task <task-attempt-id>
   ...
   * 14.-logs <job-id> <task-attempt-id>]
   */
  private void displayUsage(String cmd) {
    ...
    if ("-submit".equals(cmd)) {
      ...
      System.err.println(prefix + "[" + cmd + 
          ...
          "Valid -logsvalues for <task-state> are " + taskStates);
    } else if ("".equals(cmd)) {
      System.err.println(prefix + "[" + cmd +
