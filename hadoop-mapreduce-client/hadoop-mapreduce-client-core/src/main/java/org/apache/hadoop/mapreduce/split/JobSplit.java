...
public class JobSplit {
  static final int META_SPLIT_VERSION = 1;//版本号
  static final byte[] META_SPLIT_FILE_HEADER;//魔字符
  static {
    try {
   ...
   * JobTracker creates
   * 每一个split对象就是一个task任务对象,即每一个task去执行一个split文件
   */
  public static class TaskSplitMetaInfo {
    private TaskSplitIndex splitIndex;//包含job.split文件内容,以及该split在该job.split文件中的开始位置偏移量
    private long inputDataLength;//该split在job.split文件中所占length,即字节大小
    private String[] locations;//该split文件所在的节点路径
  ...
  public static class TaskSplitIndex {
    private String splitLocation;//job.split文件
    private long startOffset;//该task任务对应的split文件在job.split文件中的偏移量
