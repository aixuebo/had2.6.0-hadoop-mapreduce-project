 ...
 * {@link MapFile}s.
 * MapFile是排序后的SequenceFile,因此该值也是SequenceFile文件存储输出,也是根据key-value进行序列化反序列化
 */
...
public class MapFileOutputFormat 
   ...
   * 为每一个dir对应的文件生成一个MapFile.Reader对象
   * 因此组装成数组
   * */
   ...
   * 1.根据k,V,partitioner进行获取该key-value属于哪个MapFile.Reader
   * 2.从该MapFile.Reader中获取该key-value记录
   * */
