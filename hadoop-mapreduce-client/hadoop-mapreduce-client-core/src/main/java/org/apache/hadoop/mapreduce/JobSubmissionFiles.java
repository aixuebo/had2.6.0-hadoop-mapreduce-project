...
public class JobSubmissionFiles {
  ...
  // job submission directory is private! job目录权限,只能自己所属者拥有权限
  final public static FsPermission JOB_DIR_PERMISSION =
    FsPermission.createImmutable((short) 0700); // rwx--------
  //job files are world-wide readable and owner writable jo的文件权限,不能有执行权限,所属者可以写权限,其他人只能有读权限
  final public static FsPermission JOB_FILE_PERMISSION = 
    FsPermission.createImmutable((short) 0644); // rw-r--r--
  
  //return jobSubmissionDir/job.split文件
