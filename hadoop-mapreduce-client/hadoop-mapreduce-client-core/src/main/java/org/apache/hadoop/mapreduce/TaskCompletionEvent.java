...
public class TaskCompletionEvent implements Writable{
  ...
  private int eventId; 
  private String taskTrackerHttp;//http location of the tasktracker where this task ran.  该任务运行的tasktracker的http地址
  private int taskRunTime; // using int since runtime is the time difference, Returns time (in millisec) the task took to complete. 该尝试任务运行到结束花费的时间毫秒数
  private TaskAttemptID taskId;//尝试任务ID,因此可以得到任务ID
  Status status; //该尝试任务最后完成时候的状态
  boolean isMap = false;//该尝试任务是m还是r类型的任务
