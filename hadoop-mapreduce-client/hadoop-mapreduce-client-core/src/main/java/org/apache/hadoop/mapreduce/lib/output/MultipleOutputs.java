 ...
 * class.
 * </p>
 * 每一个输出目录都要配置对应的OutputFormat、key的class、value的class
 * 
 * <p>
 ...
 * </p>
 * 多个文件输出流都支持count统计,默认count统计是关闭的,该count统计的gourp组名称是类名字,count名字是每一个输出目录的名字
 * 该统计多少条记录进入了该输出目录
 * 
 * Usage pattern for job submission:使用例子
 * <pre>
 *
 * Job job = new Job();
 *
 ...
 * 定义个输出流文件叫text,是TextOutputFormat、LongWritable、Text类型的
 * MultipleOutputs.addNamedOutput(job, "text", TextOutputFormat.class,
 * LongWritable.class, Text.class);
 * 定义第二个输出流文件是seq,格式类型是SequenceFileOutputFormat、LongWritable、Text
 ...
 * </pre>
 * <p>
 * Usage in Reducer:reduce中要实现该方法
 * <pre>
 ...
 * }
 *
 * 要在输出中描述好向哪个文件去写入key-value信息
 * public void reduce(WritableComparable key, Iterator&lt;Writable&gt; values,
 * Context context)
 * throws IOException {
 * ...
 * mos.write("text", , key, new Text("Hello"));//文件名、key、value
 * mos.write("seq", LongWritable(1), new Text("Bye"), "seq_a");//文件名 key value 
 * mos.write("seq", LongWritable(2), key, new Text("Chau"), "seq_b");
 * mos.write(key, new Text("value"), generateFileName(key, new Text("value")));//key、value、文件名
 * ...
 * }
 *
...
public class MultipleOutputs<KEYOUT, VALUEOUT> {
  ...
  private static final String MULTIPLE_OUTPUTS = "mapreduce.multipleoutputs";//文件名集合,按文件名集合用空格拆分

  private static final String MO_PREFIX = 
    "mapreduce.multipleoutputs.namedOutput.";//文件名对应的classFormat的前缀

  private static final String FORMAT = ".format";//文件名对应的classFormat的后缀
  private static final String KEY = ".key";//根据前缀+namedOutput+.key后缀,获取输出流中的key对应的对象Class
  private static final String VALUE = ".value";//根据前缀+namedOutput+value后缀,获取输出流中的value对应的对象Class
  private static final String COUNTERS_ENABLED = 
    "mapreduce.multipleoutputs.counters";//是否启动count统计
   ...
   * key是nameOutput,value是该输出目录对应的TaskAttemptContext对象
   */
   ...
   * 校验文件名称必须是用大小写字母+数组组成,不允许其他字符,但是注意不允许名称为part,因为他与基础类命名冲突
   ...
   * @throws IllegalArgumentException if the output name is not valid.
   * 检查文件名是否是基础输出文件名part,不允许输出名是该名称
   */
   ...
   *
   * @param namedOutput named output Name
   * @param alreadyDefined true表示不允许重复定义名称,即namedOutput不允许已经被定义,false表示文件名不允许不存在在集合中
   * @throws IllegalArgumentException if the output name is not valid.
   * 校验文件名是否允许
   */
  private static void checkNamedOutputName(JobContext job,
    ...
    //按照空格拆分mapreduce.multipleoutputs配置信息,返回拆分后的集合
    List<String> definedChannels = getNamedOutputsList(job);
    
    if (alreadyDefined && definedChannels.contains(namedOutput)) {//alreadyDefined true表示不允许重复定义名称,即namedOutput不允许已经被定义
      throw new IllegalArgumentException("Named output '" + namedOutput +
        "' already alreadyDefined");
    } else if (!alreadyDefined && !definedChannels.contains(namedOutput)) {//alreadyDefined false表示文件名不允许不存在在集合中
      throw new IllegalArgumentException("Named output '" + namedOutput +
        "' not defined");
    }
  }

  // Returns list of channel names.按照空格拆分mapreduce.multipleoutputs配置信息,返回拆分后的集合
  ...
  // Returns the named output OutputFormat. 根据前缀+namedOutput+后缀,获取该namedOutput对应的OutputFormat对象
  ...
  // Returns the key class for a named output.根据前缀+namedOutput+key后缀,获取输出流中的key对应的对象Class
  ...
  // Returns the value class for a named output. 根据前缀+namedOutput+value后缀,获取输出流中的value对应的对象Class
   ...
   * 添加一个输出文件
   * 设置该输出文件对应的key、value、outputFormatClass信息
   */
  @SuppressWarnings("unchecked")
  ...
  private static class RecordWriterWithCounter extends RecordWriter {
    private RecordWriter writer;//输出流对象
    private String counterName;//统计名称
  ...
  private Set<String> namedOutputs;//输出文件名集合
  private Map<String, RecordWriter<?, ?>> recordWriters;//key是输入文件名,value是该文件名对应的输出流对象
  private boolean countersEnabled;//是否开启了count统计
  ...
  public MultipleOutputs(
    ...
    namedOutputs = Collections.unmodifiableSet(
      new HashSet<String>(MultipleOutputs.getNamedOutputsList(context)));//初始化文件名集合
   ...
   * 
   * @param namedOutput the named output name 文件名
   * @param key         the key
  ...
  // MultithreadedMapper.返回该文件对应的输出流
  @SuppressWarnings("unchecked")
  private synchronized RecordWriter getRecordWriter(
    ...
    // If not in cache, create a new one 没有该目录对应的输出流则创建一个
    if (writer == null) {
   ...
   // output format and output key/value types put in the context 根据文件名获取或者创建TaskAttemptContext对象
