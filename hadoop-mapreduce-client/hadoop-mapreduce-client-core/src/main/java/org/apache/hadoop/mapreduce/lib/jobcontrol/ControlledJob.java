...
public class ControlledJob {
  ...
  private List<ControlledJob> dependingJobs;//当依赖的job都执行完后,会是READY状态
   ...
   */
   synchronized State checkState() throws IOException, InterruptedException {
    ...
    for (int i = 0; i < n; i++) {
      ...
      // pred must be in success state 该job的所有依赖的都是成功的,因此设置该job为ready状态
      if (i == n - 1) {
        this.state = State.READY;
      }
    }
