 ...
 * -n, (Sort numerically)particularparticular
 ...
 *  因为partition是按照KeyFieldBasedPartitioner拆分的,因此也应该让reduce按照这样的结果进行排序,因此该类被设置目的是让reduce按照同样拆分原理进行排序
...
public class KeyFieldBasedComparator<K, V> extends WritableComparator 
  ...
  public void setConf(Configuration conf) {
    this.conf = conf;
    String option = conf.get(COMPARATOR_OPTIONS);//规则字符串
