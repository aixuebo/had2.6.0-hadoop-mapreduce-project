 ...
 * 为缓存文件和归档文件设置属性，比如文件大小、文件最后修改的时间戳、以及文件可见性
 */
...
public class ClientDistributedCacheManager {
  ...
  public static void determineCacheVisibilities(Configuration job,
    ...
    URI[] tarchives = DistributedCache.getCacheArchives(job);
    if (tarchives != null) {
    	//为每一个缓存文件判断是否是public,是则写入true,否则写入false
   ...
   * 查询url在操作系统中的状态,并且将其加入到statCache缓存中
   */
   ...
   * 检查当前path是否有可执行权限,以及该path所有的上级path必须都得有exec权限
   * 即path以及path祖先是否有exec权限
   */
   ...
   * @return true if the path in the uri is visible to all, false otherwise
   * @throws IOException
   * 1. 查询url在操作系统中的状态,并且将其加入到statCache缓存中
   * 2.获取该文件是否有action权限
   */
