 ...
 * 因为Partitioner只是把相同的key存储到一个reduce中,而reduce是有顺序的,因此可以保证相同的key在一个reduce中是有顺序的,但是不能保证全局所有的reduce有顺序,
 * 而该类TotalOrderPartitioner通过定义key的顺序,保证全局的reduce都有顺序,如果reduce有n个,则要定义的Partitioner文件要有n-1个位置,提供key进行排序
 * 
 * 读取拆分文件,拆分文件是SequenceFile类型存储的,key是序列化对象,value是NullWritable对象
 * 拆分文件有顺序,并且拆分文件大小是reduce-1个
 */
...
public class TotalOrderPartitioner<K extends WritableComparable<?>,V>
  ...
  public void setConf(Configuration conf) {
    try {
      this.conf = conf;
      //读取拆分文件,读取拆分文件,拆分文件是SequenceFile类型存储的,key是序列化对象,value是NullWritable对象
      ...
      //对拆分的数据进行校验,1校验他的数量.2.校验他是否有顺序
      
      //拆分文件的数量必须是reduce数量-1个,因为这会是设置key分配到哪个reduce关键的一步
      ...
      //true说明是自然顺序,说明是字符串比较
      boolean natOrder =
        conf.getBoolean(NATURAL_ORDER, true);
      if (natOrder && BinaryComparable.class.isAssignableFrom(keyClass)) {//字符串比较,构建树结构
      ...
      } else {//整数比较,二分法在拆分数据集合中查找key所在区间,然后返回
  ...
  // by construction, we know if our keytype 为key寻找他是属于第几个reduce
   ...
   * 二分法在拆分数据集合中查找key所在区间,然后返回
   */
  class BinarySearchNode implements Node<K> {
    private final K[] splitPoints;//拆分数据集合
    private final RawComparator<K> comparator;//比较器
   ...
   * @param fs The file system 拆分文件所在的操作系统
   * @param p The path to read 拆分文件所在路径
   * @param keyClass The map output key class 拆分文件对应的key的类型
   * @param job The job config 配置文件
   * @throws IOException
   * 读取拆分文件,拆分文件是SequenceFile类型存储的,key是序列化对象,value是NullWritable对象
   * 返回结果是key对象集合
   */
