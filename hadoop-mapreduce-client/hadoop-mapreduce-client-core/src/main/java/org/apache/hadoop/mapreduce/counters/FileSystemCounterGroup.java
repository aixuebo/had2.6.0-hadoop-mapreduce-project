...
public abstract class FileSystemCounterGroup<C extends Counter>
  ...
  //每一个groupname做key,value是该group对应的count集合,Object[]的size取决于FileSystemCounter的个数
  ...
  public static class FSCounter extends AbstractCounter {
    final String scheme;//该count的name
  ...
  //参数格式groupName_FileSystemCounter,返回结果[groupName_,FileSystemCounter]
  ...
  /**
   * 根据groupName,查找对应的count对象,即FileSystemCounter之一
   * @see org.apache.hadoop.mapreduce.counters.CounterGroupBase#findCounter(java.lang.String, boolean)
   */
  @Override
  ...
  public synchronized C findCounter(String scheme, FileSystemCounter key) {
    //检查scheme是否超限,如果没超限,返回第一次设置的scheme
    final String canonicalScheme = checkScheme(scheme);
    Object[] counters = map.get(canonicalScheme);//该groupname对应的count集合
    int ord = key.ordinal();
    if (counters == null) {
  ...
  private String checkScheme(String scheme) {
    String fixed = scheme.toUpperCase(Locale.US);//当前scheme值
    String interned = schemes.putIfAbsent(fixed, fixed);//schemes中存储的原始scheme值
    ...
    //如果schemes中以前没有该scheme,则返回当前参数scheme,而如果以前schemes中有该scheme,则返回以前的scheme
