...
abstract class BasicTypeSorterBase implements BufferSorter {
  ...
  protected int[] startOffsets; //the array used to store the start offsets of
                                //keys in keyValBuffer 每一个key/value对的开始位置
  protected int[] keyLengths; //the array used to store the lengths of
                              //keys key的长度
  protected int[] valueLengths; //the array used to store the value lengths value的长度 
  ...
  protected int count; //the number of key/values 多少个key/value对
  //the overhead of the arrays in memory 
  //12 => 4 for keyoffsets, 4 for keylengths, 4 for valueLengths, and
  //4 for indices into startOffsets array in the 是pointers、valueLengths、keyLengths、startOffsets每一个位置的字节总数,即4 + 4 + 4 + 4 = 16 
  ...
  //accurately.最大的key的长度和最大的value的长度
  ...
  /**
   * @recordOffset 该key/value的记录开始偏移量
   * @keyLength key的长度
   * @valLength value的长度
   */
  ...
  /**
   * 需要的内存大小
   * 数组多少位 * 16 即数组需要占用的空间。 + maxKeyLength + maxValLength
   */
...
//按照顺序一个一个key/value的输出
class MRSortResultIterator implements RawKeyValueIterator {
  
  private int count;
  private int[] pointers;//按顺序存储了 startOffsets偏移量的位置
  private int[] startOffsets;
  private int[] keyLengths;
  private int[] valLengths;
  private int currStartOffsetIndex;//下一个偏移量位置
  private int currIndexInPointers;//第几个pointers
