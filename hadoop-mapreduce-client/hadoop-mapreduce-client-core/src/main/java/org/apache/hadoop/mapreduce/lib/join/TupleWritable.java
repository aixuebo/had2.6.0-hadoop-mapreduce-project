 ...
 * 2.迭代序列化对象
 * 
 * 如果删除元组中的元素,其实不是真正的要移走该元素.而是仅仅在bit位置上设置移除而已
 */
  ...
  public Iterator<Writable> iterator() {
    ...
    return new Iterator<Writable>() {
      ...
      public Writable next() {
        ...
        bitIndex = written.nextSetBit(bitIndex+1);//获取下一个位置
        return t.get(returnIndex);//获取该位置的序列化对象
      }
      //不是真正的要移走该元素.而是仅仅在bit位置上设置移除而已
      public void remove() {
        if (!written.get(bitIndex)) {//如果bit位置不存在,则抛异常
