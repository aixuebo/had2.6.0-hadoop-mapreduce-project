 ...
 * method. 
 *  除了存储TaskId外,还多存储了自己的ID,表示属于该任务的第几个尝试任务
 * @see JobID
 * @see TaskID
 */
...
public class TaskAttemptID extends org.apache.hadoop.mapred.ID {
  ...
  public void readFields(DataInput in) throws IOException {
    super.readFields(in);//读取尝试ID
    taskId.readFields(in);//读取taskId
  }
   ...
   * 先比较taskId,在taskId相同的情况下,比较尝试ID
   * */
  @Override
   ...
   * 将字符串 解析成TaskAttemptID对象,即反序列化的过程
   */
