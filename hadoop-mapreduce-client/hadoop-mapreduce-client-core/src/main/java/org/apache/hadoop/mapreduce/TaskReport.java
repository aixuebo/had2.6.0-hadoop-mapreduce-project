...
/** A report on the state of a task. 报告一个任务相关进度、状态等信息*/
@InterfaceAudience.Private
@InterfaceStability.Evolving
public class TaskReport implements Writable {
  private TaskID taskid;
  private float progress;
  private String state;// The most recent state, reported by the Reporter,报告的最近一次状态
  private String[] diagnostics;
  private long startTime; 
  private long finishTime; 
  private Counters counters;
  private TIPStatus currentStatus;//该任务的当前状态对象
  
  //运行的尝试任务集合
  private Collection<TaskAttemptID> runningAttempts = 
    new ArrayList<TaskAttemptID>();
  //已经成功的尝试任务,因为只能有一个尝试任务被执行成功
