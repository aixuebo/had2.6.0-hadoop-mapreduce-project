...
public class Job extends JobContextImpl implements JobContext {  
  ...
  //调用客户端的方法,获取该jobId对应的状态,两次获取状态的时间差最大值
  ...
  public static final String COMPLETION_POLL_INTERVAL_KEY = 
    "mapreduce.client.completion.pollinterval";//监听是否完成job的时间间隔
  
  /** Default completionPollIntervalMillis is 5000 ms. 监听是否完成job的时间间隔默认值*/
  ...
  private JobStatus status;//当前或者最近一次获取的该job的运行状态
  private long statustime;///上次通过集群客户端获取该job状态的时间
  private Cluster cluster;//客户端
  private ReservationId reservationId;//返回该job预约的信息
  ...
  private void ensureState(JobState state) throws IllegalStateException {
    if (state != this.state) {//必须校验参数属性是当时的属性
      throw new IllegalStateException("Job in state "+ this.state + 
                                      " instead of " + state);
    }

    if (state == JobState.RUNNING && cluster == null) {//运行期间,必须存在集群对象
  ...
  /**
   * 主动调用客户端的方法,获取该jobId对应的状态,前提必须是运行期的job才调用该方法
   */
  ...
  String getTaskFailureEventString() throws IOException,
    ...
    int failCount = 1;
    TaskCompletionEvent lastEvent = null;//失败的任务
    ...
    if (lastEvent == null) {//没有失败的任务,可能任务失败有其他原因,只是原因没有被打入到日志中
   ...
   *  返回该jobId对应的TaskType类型任务的所有任务报告
   ...
   * @throws IOException
   * 先抓取10个,然后强转成org.apache.hadoop.mapred.TaskCompletionEvent
   */
  ...
  /**
   * 连接,产生一个Cluster对象,他是ClientProtocol客户端的意思,可以连接服务器
   * @throws IOException
   * @throws InterruptedException
  ...
  public void submit() 
    ...
    //创建集群对象连接服务器的连接
    connect();
    //根据文件系统,以及客户端ClientProtocol对象,创建JobSubmitter对象,调用submitJobInternal方法
   ...
   *         可以先提交
   *         返回值是true,表示成功完成,false表示不是成功完成,可能是kill,也可能是fault
   */
   ...
   * 监听job,并且时刻打印job信息,这些信息都是在运行job的节点上打印出来的
   */
  public boolean monitorAndPrintJob() 
      throws IOException, InterruptedException {
    String lastReport = null;//上一次报道的信息
    Job.TaskStatusFilter filter;
    Configuration clientConf = getConfiguration();
    //获取什么类型的task任务完成后,会被打印
    filter = Job.getTaskOutputFilter(clientConf);
    JobID jobId = getJobID();
    LOG.info("Running job: " + jobId);
    int eventCounter = 0;//抓取的task开始坐标
    //profile性能文件测试功能
  ...
  /**
   * 对完成的task任务事件进行打印
   * @param events 已经完成的task任务事件
   * @param filter 什么类型的task任务完成后,会被打印
   * @param profiling 性能相关测试
   * @param mapRanges map阶段的性能相关测试
   * @param reduceRanges reduce阶段的性能相关测试
   * @throws IOException
   * @throws InterruptedException
   */
  ...
  /** The interval at which monitorAndPrintJob() prints status 计算每次报道job运行情况的时间间隔*/
