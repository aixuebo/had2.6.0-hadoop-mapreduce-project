...
class JobSubmitter {
  ...
  private FileSystem jtFs;//提交的job所在文件操作系统
  private ClientProtocol submitClient;//客户端,可以获取服务器上相应接口信息
  ...
  throws ClassNotFoundException, InterruptedException, IOException {
    ...
    if (ip != null) {
      submitHostAddress = ip.getHostAddress();//job提交时候的机器的 ip.getHostAddress();
      submitHostName = ip.getHostName();//job提交时候的机器的 ip.getHostName();
