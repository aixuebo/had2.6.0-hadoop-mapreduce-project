...
public class FieldSelectionHelper {
  ...
  public static final String DATA_FIELD_SEPERATOR = 
    "mapreduce.fieldsel.data.field.separator";//拆分每一行字符串的分隔符
  public static final String MAP_OUTPUT_KEY_VALUE_SPEC = 
    "mapreduce.fieldsel.map.output.key.value.fields.spec";//map的输出的key-value格式,key-value用:拆分
  public static final String REDUCE_OUTPUT_KEY_VALUE_SPEC = 
    "mapreduce.fieldsel.reduce.output.key.value.fields.spec";//reduce的输出的key-value格式,key-value用:拆分


  /**
   * Extract the actual field numbers from the given field specs.
   * 根据从给定的field的格式中提取真实的位置数字
   * If a field spec is in the form of "n-" (like 3-), then n will be the 
   * return value. Otherwise, -1 will be returned.  
   * 如果field的格式是n-,例如3-,他只是会对value处理存在这样的格式
   * -n,例如-3,他会返回-1
  ...
  /**
   * 按照:拆分,前者是key,后者是vlue,然后key-value都按照,逗号拆分,最后形成key的数组和value的数组
   * @param keyValueSpec 例子6,5,1-3:0-
   * @param keyFieldList 
   * @param valueFieldList
   * @return 返回value中存在n-,返回n 即返回从什么地方开始一直到结束
   */
