...
public class LocatedFileStatusFetcher {
  ...
  private final Path[] inputDirs;//输入源集合
  private final PathFilter inputFilter;//文件条件过滤器
  private final Configuration conf;
  private final boolean recursive;//是否递归文件夹
  private final boolean newApi;//是否使用新api方法
  
  private final ExecutorService rawExec;//线程池
  private final ListeningExecutorService exec;//对线程池执行的线程
  private final BlockingQueue<List<FileStatus>> resultQueue;//队列,最终结果存储在该队列中
  private final List<IOException> invalidInputErrors = new LinkedList<IOException>();//异常集合
